    ...
    df = (
        lazy_df.with_columns(
            [
                ...
                .fill_null(1)
                .alias("n_chains"),
                pl.sum_horizontal([pl.col(c).is_not_null().cast(pl.Int32) for c in sequence_cols]).alias(
                    "actual_seq_count"
                ),
            ]
        )
        .with_columns(
            [
                ...
                pl.concat_str([pl.col(c).fill_null("") for c in sequence_cols], separator=":")
                .str.strip_chars(":")
                .alias("Full_Protein_Sequence"),
            ]
        )
        .with_columns([pl.col("Full_Protein_Sequence").str.to_uppercase().alias("Full_Protein_Sequence")])
        .with_columns(
